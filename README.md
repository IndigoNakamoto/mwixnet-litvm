...
## Public testnet live (as of 2026-04-15)

LitVM testnet (chain ID **4441**) is live.

- **RPC**: https://liteforge.rpc.caldera.xyz/http  
- **WS**: wss://liteforge.rpc.caldera.xyz/ws  
- **MwixnetRegistry**: `0x01bD8C4FCa29cDdd354472B3F31ef243ba92FfE7`  
- **GrievanceCourt**: `0xc303368899eAc7508cFdaAEDF9B8D03f75462593`

See [`deploy/litvm-addresses.generated.env`](deploy/litvm-addresses.generated.env) and [`PHASE_16_PUBLIC_TESTNET.md`](PHASE_16_PUBLIC_TESTNET.md) for `mlnd` / `mln-cli` configuration.

This is the judicial layer only (staking, registration, grievances). MWEB CoinSwap path is still operator-driven via `coinswapd-research` + Tor (see Phase 3 operator playbook).
...
**Production disclaimer:** While this repository’s **Phases 1–16** implementation PoC is **feature-complete in-tree** for documented bring-up paths, it is **not production** until an independent **security audit** and **production-ready patched [`coinswapd`](https://github.com/ltcmweb/coinswapd) / MWEB integration** (Go ltcmweb reference; in-repo fork at [`research/coinswapd/`](research/coinswapd/)) are finalized. **LitVM testnet** deployment of the judicial contracts is documented above; confirm RPC / explorer URLs with [LitVM docs](https://docs.litvm.com/get-started-on-testnet/add-to-wallet) if they change.
...
- **[x] Phase 1 — LitVM testnet broadcast** — **2026-04-15:** `MwixnetRegistry` + `GrievanceCourt` on chain **4441** (see **Public testnet live** above). Runbook: [`PHASE_16_PUBLIC_TESTNET.md`](PHASE_16_PUBLIC_TESTNET.md). Redeploy: `PRIVATE_KEY`, `RPC_URL`, optional `ETHERSCAN_API_KEY` in `contracts/.env` ([`contracts/.env.example`](contracts/.env.example)). See [`research/LITVM.md`](research/LITVM.md).
...
- **[ ] Phase 3-mix — Permissioned MWEB shuffle** — Mixnet product: three **`coinswapd`** hops, published hop directory, **same-value** batch, Tor. LitVM, Nostr, and **`mlnd` are not** on the happy path. **Proof A pass (2026-08-12):** lab shuffle on one host; dest scanned a new **9970000 sat** coin — [`LIVE_COINSWAP_ATTEMPT_2026-08-12-proof-a-pass.md`](LIVE_COINSWAP_ATTEMPT_2026-08-12-proof-a-pass.md). **Proof B software** (`-mln-submit-remote`, hop 0 `swap_swap`, [`scripts/mixnode-start.sh`](scripts/mixnode-start.sh)): taker is not a hop; **live pass** (1 LTC / published first hop / dest coin) waits on named hosts. Product success is the **new dest coin**, not **`pendingOnions == 0`**. Directory + Scout-free route: [`deploy/mixnet.directory.example.json`](deploy/mixnet.directory.example.json), `mln-cli route from-directory`. **Phase 3a (stub path complete, 2026-04-03):** MWEB handoff via **`mln-sidecar -mode=rpc`** + **`mw-rpc-stub`** verified end-to-end including **`E2E_MWEB_FULL=1`** Scout → pathfind → forger (**`mweb_submitRoute`**, **`mweb_runBatch`**, **`mweb_getRouteStatus`** / sidecar **`/v1/route/*`**, **`mln-cli forger -trigger-batch -wait-batch`**) — [`PHASE_3_MWEB_HANDOFF_SLICE.md`](PHASE_3_MWEB_HANDOFF_SLICE.md), [`scripts/e2e-mweb-handoff-stub.sh`](scripts/e2e-mweb-handoff-stub.sh). **Integration slice (2026-04-03):** **`make build-research-coinswapd`**, optional **`MWEB_RPC_BACKEND=coinswapd`** (balance smoke; default stub swap **502** expected), optional **`E2E_MWEB_FUNDED=1`** funded path (real keys / exact UTXO / **`swapX25519PubHex`**; dev-only **`E2E_MWEB_FUNDED_DEV_CLEAR`** for queue-clear without finalize — see Phase 3 doc), Tor-shaped URL normalization.
- **[ ] Phase 3-gov — Discovery + court** — Nostr ads, Scout, LitVM register/slash. **Parked** until you unpark it after Proof A (now a pass). Unpermissioned makers without a working shuffle only add griefing.
...
**Permanent MWEB handoff regression anchor:** before merging a PR that changes **`mln-sidecar/`**, **`research/coinswapd/`**, or **`mln-cli`** forger/sidecar-related code, and before every **`v*`** tag, run **`E2E_MWEB_FULL=1 ./scripts/e2e-mweb-handoff-stub.sh`** plus (on a suitable host) the **`MWEB_RPC_BACKEND=coinswapd`** variant with full **`COINSWAPD_FEE_MWEB`** — see [`PHASE_3_MWEB_HANDOFF_SLICE.md`](PHASE_3_MWEB_HANDOFF_SLICE.md) (*Permanent regression anchor*). When you have a funded mainnet MWEB wallet on that host, optionally run **`E2E_MWEB_FUNDED=1`** (and **`E2E_MWEB_FUNDED_DEV_CLEAR=1`** only for dev queue-clear behavior) per the same playbook. Also run Nostr wire hygiene: **`python3 nostr/validate_fixtures.py && python3 nostr/check_wire_helpers.py`**.
...
1. **Phase 3-mix (permissioned shuffle, not Scout)** — Copy [`deploy/mixnet.directory.example.json`](deploy/mixnet.directory.example.json), run three **`coinswapd-research`** mixnodes + Tor HS (**no `mlnd`**), `mln-cli route from-directory`, submit without LitVM fields. Playbook: [`research/PHASE_3_OPERATOR_PLAYBOOK.md`](research/PHASE_3_OPERATOR_PLAYBOOK.md) Part B0 / B1. **Other hop hosts from this Mac:** [`research/PHASE_3_PROOF_B_HOSTS.md`](research/PHASE_3_PROOF_B_HOSTS.md). Checklist: [`research/PHASE_3_OPERATOR_CHECKLIST.md`](research/PHASE_3_OPERATOR_CHECKLIST.md). **`make phase3-operator-preflight`**. Proof A vs Proof B labels stay distinct. **3-gov** (Nostr / LitVM / `mlnd` ads) stays parked.
2. **LitVM testnet** — **Public testnet live** (chain **4441**) is summarized at the top of this README. Operators: merge [`deploy/litvm-addresses.generated.env`](deploy/litvm-addresses.generated.env) into `deploy/.env.testnet`, set **`MLND_WS_URL`** (WebSocket), then `docker compose -f deploy/docker-compose.testnet.yml up -d`. Redeploy contracts: [`PHASE_16_PUBLIC_TESTNET.md`](PHASE_16_PUBLIC_TESTNET.md) section 0 (`make broadcast-litvm`, `make record-litvm-deploy`). Confirm [wallet / RPC params](https://docs.litvm.com/get-started-on-testnet/add-to-wallet) if they change.
...
5. **Phase 3a — MWEB handoff (stub verified 2026-04-03)** — Re-run anytime: [`PHASE_3_MWEB_HANDOFF_SLICE.md`](PHASE_3_MWEB_HANDOFF_SLICE.md), **`./scripts/e2e-mweb-handoff-stub.sh`** or **`E2E_MWEB_FULL=1`** for full CLI path (forger uses **`-trigger-batch -wait-batch`** in the script). **Research fork smoke:** **`make build-research-coinswapd`**, then set **`COINSWAPD_FEE_MWEB`** to your **full** mainnet MWEB stealth string (**`ltcmweb1`** prefix per ltcmweb/ltcd, not `mweb1`) and run **`MWEB_RPC_BACKEND=coinswapd ./scripts/e2e-mweb-handoff-stub.sh`**. **Funded path:** **`E2E_MWEB_FUNDED=1`** with **`MWEB_SCAN_SECRET`**, **`MWEB_SPEND_SECRET`**, **`E2E_MWEB_DEST`**, **`E2E_MWEB_AMOUNT`** (exact coin), optional **`E2E_MWEB_FUNDED_DEV_CLEAR=1`** — see Phase 3 playbook (no truncated `…` / `...` in addresses).
6. **Phase 2 (v1 shipped)** — When LitVM addresses change, update operator env, taker Scout settings, and optionally [`nostr/fixtures/valid/`](nostr/fixtures/valid/) for CI goldens ([`PHASE_2_NOSTR.md`](PHASE_2_NOSTR.md)). Run `python3 nostr/validate_fixtures.py` and `make test-full-stack` after wire changes.
...
| [`CONTRIBUTING.md`](CONTRIBUTING.md) | Contributing workflow, layer boundaries, saved plans under `.cursor/plans/`, Cursor / external model handoff |
...
| [`research/PHASE_3_OPERATOR_PLAYBOOK.md`](research/PHASE_3_OPERATOR_PLAYBOOK.md) | Phase 3 playbook: Part A stub, Part B0 permissioned mixnet (no Scout/`mlnd`), Part B1 Proof B, Part B Scout-era Tor |
| [`research/PHASE_3_PROOF_B_HOSTS.md`](research/PHASE_3_PROOF_B_HOSTS.md) | Proof B: step-by-step from the Mac that has MWEB to three hop computers (binary, Tor HS, directory, taker submit, hop 0 batch) |
| [`research/PHASE_3_OPERATOR_CHECKLIST.md`](research/PHASE_3_OPERATOR_CHECKLIST.md) | Phase 3-mix operator sequencing: Tor/proxy, 3× `coinswapd` hops, dest-coin success bar (court is 3-gov) |
| [`research/PHASE_3_TOR_OPERATOR_LAB.md`](research/PHASE_3_TOR_OPERATOR_LAB.md) | Live Tor + multi-hop `coinswapd` lab: **`scripts/tor-preflight.sh`**, **`HTTP_PROXY`/`socks5h`**; 3-mix vs 3-gov |
