...
4. **Phase 3a — MWEB handoff (stub verified 2026-04-03)** — Re-run anytime: [`PHASE_3_MWEB_HANDOFF_SLICE.md`](PHASE_3_MWEB_HANDOFF_SLICE.md), **`./scripts/e2e-mweb-handoff-stub.sh`** or **`E2E_MWEB_FULL=1`** for full CLI path. **Research fork smoke:** **`make build-research-coinswapd`**, then set **`COINSWAPD_FEE_MWEB`** to your **full** mainnet MWEB stealth string (**`ltcmweb1`** prefix per ltcmweb/ltcd, not `mweb1`) and run **`MWEB_RPC_BACKEND=coinswapd ./scripts/e2e-mweb-handoff-stub.sh`** (see playbook; no truncated `…` / `...` in the address).
...
| [`PHASE_3_MWEB_HANDOFF_SLICE.md`](PHASE_3_MWEB_HANDOFF_SLICE.md) | Phase 3a: `mln-sidecar -mode=rpc` + `mweb_*` stub or **`coinswapd-research`** (`MWEB_RPC_BACKEND=coinswapd` smoke **passed** 2026-04-03); stub + **`E2E_MWEB_FULL`** verified; Tor URL normalization; gaps table |
