...
- **Proof B software:** Taker `-mln-submit-remote` builds the onion and `swap_swap`s published hop 0 (no local mix queue). Directory mixnodes keep `mlnPeers` on `Swap`. One-hop [`scripts/mixnode-start.sh`](scripts/mixnode-start.sh) has no scan/spend. Live 1 LTC pass waits on named hosts. Host bring-up from the wallet Mac: [`research/PHASE_3_PROOF_B_HOSTS.md`](research/PHASE_3_PROOF_B_HOSTS.md).
