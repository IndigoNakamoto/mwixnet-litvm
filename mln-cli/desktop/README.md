...
Phase 11 taker GUI: same stack as `mln-cli` (scout → pathfind → forger) with settings persisted under the OS user config directory (`mln-wallet/settings.json`). The mixnet happy path is `coinswapd` + hop directory; this GUI must not grow Scout/LitVM/Solidity as mix execution. Proof B taker sidecar talks to **local** `-mln-submit-remote` coinswapd, not hop 0. The balance panel calls **`GET /v1/balance`** on your MLN sidecar (see [`PHASE_10_TAKER_CLI.md`](../../PHASE_10_TAKER_CLI.md) and the frozen taker contract in [`mln-sidecar/README.md`](../../mln-sidecar/README.md)). For local E2E, start **`mln-sidecar`** via [`deploy/docker-compose.e2e.yml`](../../deploy/docker-compose.e2e.yml) (port **8080**) or `make build-mln-sidecar`; in production, implement the same contract on your `coinswapd` fork or proxy. **Self-Included Routing** (optional N2 = your maker) is described in [`PHASE_14_SELF_INCLUSION.md`](../../PHASE_14_SELF_INCLUSION.md).
