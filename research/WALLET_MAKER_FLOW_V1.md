...
- implEpoch schedule (for operator planning): same v1 default as taker doc — daily cutover at `00:00:00 UTC` (see `[research/WALLET_TAKER_FLOW_V1.md](WALLET_TAKER_FLOW_V1.md)`).
...
- Flow covers setup, advertise, operate, exit, and grievance awareness.
- UTC-midnight scheduling is consistent with the taker v1 doc.
- Edge cases include operator message and deterministic action.
- No new protocol rail (MWEB fees happy path; LitVM judicial + registry only).
- Traceable to current product spec, user stories, Nostr profile, and contracts.
