...
func main() {
	...
	scanKey, err := decodeSecretKeyHex("mweb-scan-secret", *mwebScanSecret)
	if err != nil {
		return
	}
	spendKey, err := decodeSecretKeyHex("mweb-spend-secret", *mwebSpendSecret)
	if err != nil {
		return
	}
	mwebSvc := &mwebService{
	...
	if err = rpcServer.RegisterName("mweb", mwebSvc); err != nil {
		return
	}
...
var errNotNodeZero = errors.New("node index is not zero")

func (s *swapService) acceptOnionLocked(o *onion.Onion) error {
	if s.nodeIndex != 0 {
		return errNotNodeZero
	}
	if err := validateOnion(o); err != nil {
		return err
	}
	if err := saveOnion(db, o); err != nil {
		return fmt.Errorf("save onion: %w", err)
	}
	return nil
}
