...
| Phase 16 public testnet readiness (RPC_URL deploy, verification env, operator compose without Anvil/local relay) | `PHASE_16_PUBLIC_TESTNET.md`, `deploy/docker-compose.testnet.yml`, `deploy/.env.testnet.example`, `contracts/.env.example`, `scripts/broadcast-litvm-testnet.sh`, `scripts/record-litvm-deploy.py`, `Makefile` (`broadcast-litvm`, `record-litvm-deploy`), `mln-cli/internal/config/settings.go` |
...
| Phase 3 live Tor + multi-hop `coinswapd` (operator lab; README Phase 3 gate with public LitVM) | `research/PHASE_3_TOR_OPERATOR_LAB.md`, `research/PHASE_3_OPERATOR_PLAYBOOK.md`, `research/PHASE_3_OPERATOR_PARTB_STEPBYSTEP.md` (Part B detail), `research/PHASE_3_OPERATOR_CHECKLIST.md`, `scripts/tor-preflight.sh`, `make tor-preflight`, `make phase3-operator-preflight` |
...
| Taker CLI (`mln-cli`); maker onboard (`mln-cli maker onboard`); Wails taker wallet (`mln-cli/desktop/`, build tag `wails`); shared maker-ad structs for `mlnd` + client; Forger → MLN HTTP sidecar (`GET /v1/balance`, `POST` route JSON, not vanilla `swap_Swap`); mock sidecar for local E2E (`mln-sidecar`); optional self-as-N2 routing (Phase 14); optional `mlnd` loopback Maker dashboard (`MLND_DASHBOARD_ADDR`); taker route artifact (`mln-cli route build` or **`route from-directory`** → `route.json`); permissioned mixnet hop list (`deploy/mixnet.directory.example.json`) | `PHASE_10_TAKER_CLI.md`, `PHASE_14_SELF_INCLUSION.md`, `mln-cli/desktop/README.md`, `mlnd/MAKER_DASHBOARD_SETUP.md`, `research/COINSWAPD_TEARDOWN.md` (sidecar + `swap_forward`), `research/COINSWAPD_MLN_FORK_SPEC.md`, `mln-cli/internal/forger/`, `mln-cli/internal/mixnetdir/`, `mln-cli/internal/takerflow/`, `mln-cli/internal/pathfind/`, `mlnd/pkg/makerad`, `mln-sidecar/` |
...
| Agent Handoff Guide — How to pair Cursor with Grok / Gemini / other models while respecting MLN boundaries and skills | `docs/AGENT_HANDOFF.md` |
...
**Phase 1 (local):** contracts, `EvidenceLib`, fuzz and invariant tests, Slither on `contracts/**` in GitHub Actions, `make contracts-test`, `scripts/deploy-local-anvil.sh` — see `README.md` roadmap. **Implementation phases 10–16** (through public testnet readiness packaging) are **shipped in-tree**, including **`mln-cli maker onboard`** and an **optional `mlnd` Maker dashboard** (loopback by default). **LitVM testnet:** `MwixnetRegistry` + `GrievanceCourt` are deployed on chain **4441** as of **2026-04-15** (`README.md` **Public testnet live**, `deploy/litvm-addresses.generated.env`); confirm RPC/WS with LitVM docs if endpoints rotate. **Phase 2** (Nostr kinds **31250–31251**, fixtures + CI, Scout deployment filters, `mlnd` ads) is **documented as v1 complete** (`research/NOSTR_MLN.md`, `PHASE_2_NOSTR.md`). **Phase 3a** (MWEB route JSON → **`mln-sidecar -mode=rpc`** → **`mweb_submitRoute`**, plus **`mweb_runBatch`** / **`mweb_getRouteStatus`** and **`mln-cli forger -trigger-batch -wait-batch`**) is **verified** on the **`mw-rpc-stub`** path with **`E2E_MWEB_FULL=1 ./scripts/e2e-mweb-handoff-stub.sh`** as of **2026-04-03** (`PHASE_3_MWEB_HANDOFF_SLICE.md` — **completed swap path achieved in-repo** on the stub; see playbook for gaps vs live P2P). **`bin/coinswapd-research`** + **`MWEB_RPC_BACKEND=coinswapd`** operator smoke **passed** the same date (balance OK; default stub **`POST /v1/swap`** **502** expected). **`E2E_MWEB_FUNDED=1`** extends that script for **real keys + UTXO + `swapX25519PubHex`** through submit → batch → status (optional **`-mweb-dev-clear-pending-after-batch`** for **`pendingOnions=0`** without finalize — DEV ONLY; see `PHASE_3_MWEB_HANDOFF_SLICE.md`). Tor-shaped URL normalization (**`mln-sidecar`**, **`mln-cli` pathfind/forger**) narrows fork bring-up. **README Phase 3** is split: **3-mix** (permissioned `coinswapd` shuffle, hop directory, same-value batches) — **Proof A pass 2026-08-12** ([`LIVE_COINSWAP_ATTEMPT_2026-08-12-proof-a-pass.md`](LIVE_COINSWAP_ATTEMPT_2026-08-12-proof-a-pass.md)); **Proof B software** (`-mln-submit-remote` / published hop 0 `swap_swap`) is in-tree; **live Proof B** (1 LTC dest coin on separate hosts) remains open. **3-gov** (Nostr discovery + LitVM slash) stays parked until explicitly unparked. Mix success is a **new dest MWEB coin**, not an empty onion queue. **Security docs:** `research/THREAT_MODEL_MLN.md` (accepted audit snapshot + threat tables), `research/RED_TEAM_MLN.md` (red-team narratives; extends the threat model).

**Phase 1 (testnet):** redeploy or refresh env using `PHASE_16_PUBLIC_TESTNET.md` section 0 when addresses or RPC change (`research/LITVM.md`).
