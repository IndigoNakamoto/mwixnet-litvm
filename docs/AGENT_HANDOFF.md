...
That split keeps always-on context small while still having deep procedures in-repo.
...
2. **External designs, Cursor executes** — In Grok/Gemini, explore options with explicit constraint: "Must respect MWEB vs LitVM separation per this paste." Paste **Layer boundaries** + one paragraph of task from `AGENTS.md`. Bring the **conclusion** back to Cursor as a short spec bullet list.
...
**Bottom line:** `.cursor` is your **scoped automation** of "read AGENTS + the right skill"; `AGENTS.md` is the **repo router and boundary contract**. Grok/Gemini are useful as **second opinions and design space**, with **you** carrying the same boundaries and canonical paths between tools.
