...
**Implementation status (this repository, relative to P0–P1 product work):** The following are **shipped in-tree** and align with the product layers above: **`mln-cli`** (Go taker client: scout, pathfind, forger), **Wails desktop wallet**, **`mln-sidecar`** HTTP bridge with **`-mode=mock`** and **`-mode=rpc`** forwarding to JSON-RPC **`mweb_submitRoute`** / **`mweb_getBalance`** (see [`mln-sidecar/README.md`](mln-sidecar/README.md)), **LitVM contract economic hardening** (Phase 15: slashing, bonds, exit locks, reentrancy guard; Foundry fuzz invariants), and **Slither** in [`.github/workflows/contracts.yml`](.github/workflows/contracts.yml). **Still open for P0-class integration:** a **`coinswapd` fork** that implements **`mweb_submitRoute`** (and the balance RPC used by the sidecar) so the MLN HTTP bridge can complete the real MWEB handoff. The public **ltcmweb** reference is **Go** ([coinswapd](https://github.com/ltcmweb/coinswapd)); vanilla RPC is **`swap_Swap(onion.Onion)`** only (see [`research/COINSWAPD_TEARDOWN.md`](research/COINSWAPD_TEARDOWN.md)). Older notes sometimes say “C++ engine”; treat the **fork contract** (`mweb_submitRoute`) as normative, not the language label. **P1 on-chain:** LitVM **public testnet broadcast** of registry + court remains pending RPC/faucet readiness (see [`README.md`](README.md)).
